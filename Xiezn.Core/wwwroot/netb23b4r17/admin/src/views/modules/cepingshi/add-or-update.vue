...
export default {
	data() {
		...
		return {
			...
			rules: {
				cepingzhanghao: [
					{ required: true, message: '测评账号不能为空', trigger: 'blur' },
				],
				mima: [
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `cepingshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.cepingshiCrossAddOrUpdateFlag = false;
											this.parent.search();
